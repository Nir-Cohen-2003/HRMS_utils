...
class SimilarityVsInfoConfig:
    ...
    Attributes:
      ...
      - library_parquet_path: Path to the source library parquet (must contain `idx`, `mol_idx`, and the information measure).
    ...
    library_parquet_path: Union[str, Path]
...
def compute_per_spectrum_avg_tanimoto(
    ...
    lib_path = Path(config.library_parquet_path)
    assert pairs_path.exists(), f"Pairs parquet not found: {pairs_path}"
    assert lib_path.exists(), f"Library parquet not found: {lib_path}"
    ...
    logger.info("Scanning library parquet: %s", lib_path)
    # Recreate row index `idx` and molecule index `mol_idx` deterministically so it
    # matches how the pairs file was generated by `build_and_write_pairs_parquet`.
    lib = (
        pl.scan_parquet(str(lib_path))
        .filter(pl.col("smiles").is_not_null())
        .with_row_index("idx")
        .cast({"idx": pl.Int64})
        .with_columns(mol_idx=pl.col("idx").min().over(["base_inchikey", "ion_mode"]))
    )
    # Require the columns that we depend on being present in the source library.
    _require_columns_or_fail(
        lib,
        ["smiles", "base_inchikey", "ion_mode", config.info_measure],
        "library parquet",
    )
...
if __name__ == "__main__":
    ...
    PAIRS_WITH_TANIMOTO_PATH = Path(
        "/home/analytit_admin/Data/spectral_libs/info_score/combined_library_pairs_with_tanimoto_full.parquet"
    )
    OUTPUT_DIR = Path(
        "/home/analytit_admin/Data/spectral_libs/info_score/sim_vs_info_analysis"
    )

    cfg = SimilarityVsInfoConfig(
        pairs_parquet_path=PAIRS_WITH_TANIMOTO_PATH,
        library_parquet_path=LIBRARY_PATH,
