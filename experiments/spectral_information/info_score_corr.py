import polars as pl
import numpy as np
from hrms_utils.spectral_information import spectral_info_score_polars
...
def main():
    ...
    # Check if old information score is present and rename it
    if "spectral_tree_info_score" in information_scores.columns:
        information_scores = information_scores.rename({
            "spectral_tree_info_score": "spectral_tree_info_score_old"
        })
        print("Found and renamed old tree score column.")

    # Always calculate the new information score
    print("Calculating new information score.")
    information_scores = information_scores.with_columns(
        pl.struct(["Formula_array", "fragment_formulas"])
          .map_batches(
              lambda s: spectral_info_score_polars(s.struct.field("Formula_array"), s.struct.field("fragment_formulas")),
              return_dtype=pl.Float64).alias("spectral_info_score")
    )

    # If old score exists, calculate and print correlation
    if "spectral_tree_info_score_old" in information_scores.columns:
        correlation = information_scores.select(
            pl.corr("spectral_tree_info_score_old", "spectral_info_score", method="spearman")
        ).item()
        print(f"Spearman correlation between old and new tree score: {correlation:.4f}")

    # 4. All-to-all spectral similarity search
    print("Running similarity search...")
    
    df1 = information_scores.select(["NIST_ID", "base_InChIKey", "masses_normalized", "cleaned_intensities", "spectral_info_score", "Formula_array"])
    ...
    if all_pairs.height > 0:
        ...
        all_results.write_parquet("all_pairs_similarity_results_tree_score.parquet")
        print(f"Saved {all_results.height} spectrum pair comparisons to all_pairs_similarity_results_tree_score.parquet")
    ...
    correlations = data_for_correlation.group_by("base_InChIKey").agg([
        pl.corr("spectral_info_score", "max_similarity", method="spearman").alias("spearman_info_score_vs_max_similarity"),
        pl.corr("spectral_info_score", "match_count_above_0p80", method="spearman").alias("spearman_info_score_vs_match_count_0p80"),
        pl.corr("spectral_info_score", "match_count_above_0p85", method="spearman").alias("spearman_info_score_vs_match_count_0p85"),
        pl.corr("spectral_info_score", "match_count_above_0p90", method="spearman").alias("spearman_info_score_vs_match_count_0p90"),
        pl.corr("spectral_info_score", "match_count_above_0p95", method="spearman").alias("spearman_info_score_vs_match_count_0p95"),
    ])

    # Why: Save comprehensive correlation results for analysis
    print("Saving results...")
    correlations.write_parquet("info_score_spearman_correlations.parquet")

    # Why: Visualize distribution of correlations for max similarity (primary metric)
    print("Generating histogram...")
    plt.figure(figsize=(10, 6))
    plt.hist(correlations["spearman_info_score_vs_max_similarity"].drop_nulls(), bins=50, alpha=0.7, label="Spectral Info Score")
    plt.xlabel("Spearman Correlation")
    plt.ylabel("Frequency")
    plt.title("Distribution of Spearman Correlations (vs Max Similarity)")
    plt.legend()
    plt.savefig("info_score_spearman_correlations_histogram.png")
    print("Histogram saved to info_score_spearman_correlations_histogram.png")
