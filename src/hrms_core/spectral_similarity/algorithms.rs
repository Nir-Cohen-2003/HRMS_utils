...
pub fn apply_weight_to_intensity(spectrum: &mut Spectrum) {
    ...
    if entropy < 3.0 && entropy >= 0.0 {
        // Added check for non-negative entropy
...
pub fn calculate_unweighted_entropy_similarity(
    ...
    while a < spec_a.peaks.len() && b < spec_b.peaks.len() {
        ...
        let tolerance = (ms2_tolerance_in_ppm * 1e-6 * MASS_THRESHOLD_FOR_PPM)
            .max(ms2_tolerance_in_ppm * 1e-6 * mass);
        ...
        if mass_diff < -tolerance {
            ...
            if intensity_a > 0.0 && intensity_b > 0.0 && intensity_ab > 0.0 {
                // Avoid log2(0)
                similarity += intensity_ab * intensity_ab.log2()
                    - intensity_a * intensity_a.log2()
                    - intensity_b * intensity_b.log2();
            }
            a += 1;
            b += 1;
        }
...
pub fn general_cosine_similarity(
    ...
    if a_cleaned.peaks.is_empty() || b_cleaned.peaks.is_empty() {
        return 0.0;
    }

    let weighted_peaks_a: Vec<(f64, f64)> = a_cleaned
        .peaks
        .iter()
        .map(|p| {
    ...
    let weighted_peaks_b: Vec<(f64, f64)> = b_cleaned
        .peaks
        .iter()
        .map(|p| {
            let weighted_intensity = p.intensity.powf(intensity_power) * p.weight.powf(mass_power);
    ...
    for (i, &(mz_a, intensity_a)) in weighted_peaks_a.iter().enumerate() {
        let tolerance = (ms2_tolerance_in_ppm * 1e-6 * MASS_THRESHOLD_FOR_PPM)
            .max(ms2_tolerance_in_ppm * 1e-6 * mz_a);
...
pub fn calculate_explained_intensity(
    ...
    if a_cleaned.peaks.is_empty() {
        return 0.0;
    }
    if b_cleaned.peaks.is_empty() {
        return 0.0;
    }
    ...
    while a_idx < weighted_peaks_a.len() && b_idx < weighted_peaks_b.len() {
        ...
        let tolerance = (ms2_tolerance_in_ppm * 1e-6 * MASS_THRESHOLD_FOR_PPM)
            .max(ms2_tolerance_in_ppm * 1e-6 * mass_a);
    ...
    let sum_b: f64 = weighted_peaks_b
        .iter()
