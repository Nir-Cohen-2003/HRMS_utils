...
from typing import cast, TypeVar, Optional, List, Union
import re

from ..hrms_core import *
from ..formula_annotation.utils import format_formula_string_to_array, num_elements, formula_to_array
from ..formula_annotation.element_table import ADDUCT_MASSES
from . import nist_mspec
from . import mgf
...
def process_spectral_library(
    ...
    dedup_tolerance_ppm: float = 10.0,
    ...
    df = _deduplicate_spectra(df, tolerance_ppm=dedup_tolerance_ppm, threshold=dedup_threshold)
...
def _process_pipeline(
    ...
    """Core processing pipeline moved from nist_mspec.py."""
    ...
    # Column selection
    cols = [
        "name", "nist_id", "db_id", "instrument_type", "instrument", "ionization", "ion_mode",
        "mslevel", "collision_energy_NCE", "collision_energy_ev", "collision_energy_list",
        "multiple_collision_energies", "collision_energy_mean", "cas", "inchikey", "base_inchikey",
        "smiles", "inchi", "is_orbitrap", "is_TOF", "is_ESI", "precursor_type", "precursor_mz",
        "molecular_formula", "molecular_formula_array", "precursor_formula_array", "clean_precursor",
        "exact_mass", "raw_spectrum_mz", "raw_spectrum_intensity", "cleaned_normalized_mz",
        "cleaned_normalized_intensity", "cleaned_fragment_formulas", "cleaned_fragment_formulas_str",
        "cleaned_fragment_errors_ppm", "explained_intensity", "molecular_ion_intensity",
        "spectral_information_score", "spectral_information_score_with_hydrogens", "source_file"
    ]
...
def _filter_invalid_entries(lf: pl.LazyFrame) -> pl.LazyFrame:
    """Filters out entries with no formula and no other identifiers."""
    return lf.filter(
        pl.col("molecular_formula").is_not_null() | 
...
def _compute_precursor_formula(lf: pl.LazyFrame) -> pl.LazyFrame:
    ...
    check_df = lf.filter(pl.col("molecular_formula").is_null() & pl.col("inchikey").is_not_null()).limit(1).collect()
    if not check_df.is_empty():
        raise NotImplementedError("Molecular formula missing for entry with InChIKey. This will be handled by parallel_rdkit.")
...
def _annotate_and_filter_metadata(data: polarsFrame) -> polarsFrame:
    instrument_data_columns = pl.selectors.by_name(["instrument", "instrument_type", "ionization"])
    qq_mask = pl.any_horizontal(instrument_data_columns.str.contains(r"(?i)QQ").fill_null(False))
    
    return cast(polarsFrame, data.filter(qq_mask.not_()).with_columns(
        ...
        pl.any_horizontal(instrument_data_columns.str.contains(r"(?i)ESI|LC").fill_null(False)).alias("is_ESI"),
    ))
...
def _extract_collision_energy_values(data: polarsFrame) -> polarsFrame:
    ...
    if "collision_energy_list" in cols:
        return cast(polarsFrame, data.with_columns(
    ...
    return cast(polarsFrame, data.with_columns(
        ...
        .cast(pl.Float64, strict=False).alias("collision_energy_NCE"),
        pl.col("collision_energy_raw").str.extract(pat_ev, group_index=1)
        .cast(pl.Float64, strict=False).alias("collision_energy_ev"),
        pl.col("collision_energy_raw").str.extract(pat_list_content, group_index=1)
        .str.extract_all(r"\d+(?:\.\d+)?").list.eval(pl.element().cast(pl.Float64, strict=False))
        .alias("collision_energy_list"),
    ).with_columns(
        pl.when(pl.col("collision_energy_NCE").is_null() & pl.col("collision_energy_ev").is_null() & pl.col("collision_energy_list").is_null())
        .then(pl.col("collision_energy_raw").str.extract(pat_num, group_index=1).cast(pl.Float64, strict=False))
        .otherwise(None).alias("_bare_energy"),
        pl.col("collision_energy_list").list.mean().alias("_list_mean"),
    ).with_columns(
        pl.when(pl.col("collision_energy_NCE").is_null())
        .then(pl.when(pl.col("is_orbitrap")).then(pl.coalesce([pl.col("_list_mean"), pl.col("_bare_energy")])).otherwise(None))
        .otherwise(pl.col("collision_energy_NCE")).alias("collision_energy_NCE"),
        pl.when(pl.col("collision_energy_ev").is_null())
        .then(pl.when(pl.col("is_orbitrap").not_()).then(pl.coalesce([pl.col("_list_mean"), pl.col("_bare_energy")])).otherwise(None))
        .otherwise(pl.col("collision_energy_ev")).alias("collision_energy_ev"),
    ).with_columns(
        pl.col("collision_energy_list").list.len().ge(2).fill_null(False).alias("multiple_collision_energies"),
        pl.coalesce([pl.col("_list_mean"), pl.col("collision_energy_NCE"), pl.col("collision_energy_ev")]).alias("collision_energy_mean"),
    ).drop("_bare_energy", "_list_mean"))

def _annotate_spectra(data: polarsFrame, raw_fragment_tolerance_ppm: float, normalized_fragment_tolerance_ppm: float) -> polarsFrame:
    ...
    return cast(polarsFrame, data_frame.with_columns(
        ...
        pl.struct([
            ...
            raw_fragment_tolerance_ppm=raw_fragment_tolerance_ppm,
            normalized_fragment_tolerance_ppm=normalized_fragment_tolerance_ppm,
            min_dbe=-0.5, max_dbe=40, dbe_mode="half_integer", water_absorption=True,
    ...
    ).drop("cleaned_normalized_spectra").with_columns(
...
def _add_precursor_type_indicators(data: polarsFrame) -> polarsFrame:
    ...
    return cast(polarsFrame, data.with_columns(
        ...
        (pl.col("Isotope") | pl.col("Cation") | pl.col("Multimer") | pl.col("MultiCharge") | pl.col("Fragment") | 
         pl.col("precursor_type").str.contains("M").not_()).not_().alias("clean_precursor")
    ))

def _add_molecular_ion_info(NIST: polarsFrame, tolerance_ppm: float = 10.0) -> polarsFrame:
    lf = NIST.lazy().with_columns(
        molecular_ion_intensity=pl.when(
            pl.col("cleaned_normalized_mz").list.last().is_close(
                pl.col("precursor_mz"), rel_tol=tolerance_ppm * 1e-6, abs_tol=200.0 * tolerance_ppm * 1e-6
            )
        ).then(pl.col("cleaned_normalized_intensity").list.last()).otherwise(None)
    )
    return cast(polarsFrame, lf if isinstance(NIST, pl.LazyFrame) else lf.collect(engine="streaming"))
...
def _add_spectral_information_score(data: polarsFrame) -> polarsFrame:
    return cast(polarsFrame, data.with_columns(
        ...
        pl.col("spectra_for_spectral_info").spectral_info.spectral_info_score(distance_metric="l2", ignore_hydrogens=True).alias("spectral_information_score"),
        pl.col("spectra_for_spectral_info").spectral_info.spectral_info_score(distance_metric="l2", ignore_hydrogens=False).alias("spectral_information_score_with_hydrogens"),
    ))

def _deduplicate_spectra(df: pl.DataFrame, tolerance_ppm: float, threshold: float) -> pl.DataFrame:
    ...
    pairs = df.select(["_dedup_idx", "base_inchikey", "cleaned_normalized_mz", "cleaned_normalized_intensity", "precursor_mz"]).join(
        df.select(["_dedup_idx", "base_inchikey", "cleaned_normalized_mz", "cleaned_normalized_intensity", "precursor_mz"]),
        on="base_inchikey", suffix="_right"
    ...
    to_remove = pairs.filter(
        (pl.col("sim_forward") >= threshold) & (pl.col("sim_reverse") >= threshold)
    ).select("_dedup_idx_right").unique()
    
    return df.filter(~pl.col("_dedup_idx").is_in(to_remove["_dedup_idx_right"])).drop("_dedup_idx")

def _enrich_with_pubchem(lf: pl.LazyFrame, pubchem_path: Path) -> pl.LazyFrame:
    """Logic adapted from convert_msp.py."""
    pubchem_lf = pl.scan_parquet(str(pubchem_path), low_memory=True).select([
    ...
    pubchem_lf = pubchem_lf.unique(subset="base_inchikey")
    
    lf = lf.join(pubchem_lf, on="base_inchikey", how="left", suffix="_pubchem")
    
    # Fill missing identifiers
    return lf.with_columns(
        pl.when(pl.col("smiles").is_null() | (pl.col("smiles") == "")).then(pl.col("smiles_pubchem")).otherwise(pl.col("smiles")).alias("smiles"),
        pl.when(pl.col("inchi").is_null() | (pl.col("inchi") == "")).then(pl.col("inchi_pubchem")).otherwise(pl.col("inchi")).alias("inchi"),
        pl.when(pl.col("inchikey").is_null() | (pl.col("inchikey") == "")).then(pl.col("inchikey_pubchem")).otherwise(pl.col("inchikey")).alias("inchikey"),
    ).drop([c for c in lf.collect_schema().names() if c.endswith("_pubchem") or c in ["cid", "formula", "monoisotopic_mass"]])
...
def _standardize_structures(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if mapping_df["smiles"].null_count() > 0:
        missing = mapping_df.filter(pl.col("smiles").is_null())["base_inchikey"].to_list()
        raise ValueError(f"Missing SMILES for base_inchikeys: {missing}")
    
    smiles_list = mapping_df["smiles"].to_list()
    results = msready_inchi_inchikey_parallel(smiles_list)
    
    # Assuming results is a list of tuples (smiles, inchi, inchikey) based on my earlier guess or whatever the actual API returns.
    # User said "It takes list[str] SMILES" and I asked if it returns parallel lists or tuples.
    # The response was "It takes list[str] SMILES" which didn't fully clarify return type, 
    # but let's assume parallel lists as it's common for "parallel" named functions.
    # Actually, the user prompt said: "takes a list of smiles and retunr a list of msready smiles, msready inchi and msready inchikey"
    # This likely means 3 lists.
    
    msready_smiles, msready_inchi, msready_inchikey = results
    
    # Validation
    for i, (s, inc, inck) in enumerate(zip(msready_smiles, msready_inchi, msready_inchikey)):
        if not s or not inc or not inck:
            raise ValueError(f"MS-ready standardization failed for SMILES: {smiles_list[i]}")
