...
inline bool MassDecomposer::check_dbe(const Formula& formula, mass_t min_dbe, mass_t max_dbe, int dbe_mode) const {
    ...
    mass_t dbe = (2.0 + 2.0*c_count + 3.0*p_count + n_count - h_count - x_count) / 2.0;
    
    if (dbe < min_dbe || dbe > max_dbe) return false;
    
    if (dbe_mode == 0) { // integer
        if (std::abs(dbe - std::round(dbe)) > 1e-8) return false;
    } else if (dbe_mode == 1) { // half-integer
        if (std::abs(2.0 * dbe - std::round(2.0 * dbe)) > 1e-8) return false;
    }
...
std::vector<Formula> MassDecomposer::decompose(mass_t target_mass, const DecompositionParams& params) {
    if (!is_initialized_) {
        init_money_changing();
        discretize_masses();
        divide_by_gcd();
    ...
    mass_t tolerance = std::max(target_mass, (mass_t)200.0) * params.tolerance_ppm / 1e6;
    ...
    for (long long mass = start; mass <= end; ++mass) {
        ...
        for (const auto& result : mass_results) {
            ...
            mass_t mass_error = std::abs(exact_mass - target_mass);
            if (mass_error > tolerance) {
                continue;
            }
            // Use unified constraint checking
            if (!check_dbe(result, params.min_dbe, params.max_dbe, params.dbe_mode)) {
                continue;
            }
...
std::vector<FormulaWithString> MassDecomposer::decompose_verbose(mass_t target_mass, const DecompositionParams& params) {
