...
pub fn calculate_similarity_struct(inputs: &[Series], kwargs: SimilarityKwargs) -> PolarsResult<Series> {
    ...
    let precursor_mz_vec: Vec<Option<f64>> = precursor_mz_ca.into_iter().collect();
    ...
    let out: Float64Chunked = mz1_vec
        ...
        .zip(precursor_mz_vec.into_par_iter())
        .map(|((((opt_mz1, opt_int1), opt_mz2), opt_int2), precursor_mz)| {
